...
class AppToast{
  ...
  Future<void> showError(BuildContext context,String message) async{
    final toast = _Toast(msg: message, duration: _duration,  icon: FontAwesomeIcons.exclamationCircle, backgroundColor: Color(0xFFf1dbda), iconColor: KColors.errorColor,);
  ...
  Future<void> showSuccess(BuildContext context,String message) async{
    final toast = _Toast(msg: message, duration: _duration,  icon: FontAwesomeIcons.check, backgroundColor: KColors.success.withOpacity(0.2), iconColor: KColors.success,);
    final oneWayAnimationCubit = _OneWayAnimationCubit();
    final f = showToastWidget(toast, position: ToastPosition.top,
        duration: _duration, handleTouch: true, dismissOtherToast: true, animationBuilder: (context,w,c,b) {
          return _OneWayOffsetAnimationBuilder(maxOffsetX: 0, maxOffsetY: -100, oneWayAnimationCubit: oneWayAnimationCubit).call(context, w, c, b);
        ...
        child: SizedBox(
          ...
          child: Row(
            ...
            children: [
              ...
              SizedBox(width: 10,),
              Expanded(child: Text(widget.msg, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: KColors.bgColor, fontSize: 14),)),
              SizedBox(width: 10,),
            ],
          ),
        ),
...
class _OneWayAnimationCubit extends Cubit<_OneWayAnimationState>{
  _OneWayAnimationCubit() : super(const _OneWayAnimationState(animatesBack: false, idle: false));
